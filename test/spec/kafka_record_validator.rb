...
class KafkaRecordValidator
  ...
  def all_notification_records(consumer, notification_topic, batch_id, expected_messages, batch_completion_delay = nil, batch_info = nil, invalid_count = nil)
    counter = 1
    messages = []
    begin
        ...
        consumer.each_message do |message|
          ...
          if batch_info
            ...
            if parsed_data['status'] == 'completed'
              raise "The expectedRecordCount (#{parsed_data['expectedRecordCount']}) does not equal the actualRecordCount (#{parsed_data['actualRecordCount']})" unless parsed_data['expectedRecordCount'] == parsed_data['actualRecordCount']
            end
          end

          if invalid_count && %w(failed completed).include?(parsed_data['status'])
            raise "Notification message contains an incorrect invalidRecordCount. Expected #{invalid_count}, Received #{parsed_data['invalidRecordCount']}" unless parsed_data['invalidRecordCount'] == invalid_count
          end
