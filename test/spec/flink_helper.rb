...
class FlinkHelper
  ...
  def start_flink_job(jar_id, input_topic, batch_completion_delay, token, parallelism = nil)
  ...
  def verify_job_state(job_id, token, state)
    ...
    state == 'RUNNING' ? Logger.new(STDOUT).info("New Job Running With ID: #{job_id}") : Logger.new(STDOUT).info("Test Job ID #{job_id} successfully canceled")
  end
  ...
  def upload_jar_from_dir(jar_name, from_dir, flink_oauth_token)
    ...
    original_jar_name = Dir.entries(File.join(File.dirname(__FILE__), from_dir)).select { |n| n.match(/hri-flink-validation-fhir-.+.jar/) }[0]
