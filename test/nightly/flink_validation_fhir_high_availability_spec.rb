...
#   - run `./gradlew build publishToMavenLocal` in the project's root directory to build the flink-validation-fhir jar
...
describe 'Flink Validation FHIR High Availability Job' do
  ...
  before(:all) do
    ...
    @output_consumer_group = "flink-validation-fhir-#{@travis_branch}-#{timestamp}-output-consumer"
    @notification_consumer_group = "flink-validation-fhir-#{@travis_branch}-#{timestamp}-notification-consumer"
    @invalid_consumer_group = "flink-validation-fhir-#{@travis_branch}-#{timestamp}-invalid-consumer"
    @kafka = Kafka.new(ENV['KAFKA_BROKERS'], client_id: "flink-validation-fhir-#{@travis_branch}-#{timestamp}", connect_timeout: 10, socket_timeout: 10, sasl_plain_username: 'token', sasl_plain_password: ENV['SASL_PLAIN_PASSWORD'], ssl_ca_certs_from_system: true)
  ...
  after(:all) do
    begin
      ...
      response = @elastic.es_delete_by_query(TENANT_ID, "name:flink-validation-fhir-#{ENV['TRAVIS_BRANCH']}*")
  ...
  it 'should output all successfully validated records with the same key, headers, and body after deleting a flink taskmanager pod' do
      ...
      batch_template = {
          name: "flink-validation-fhir-#{ENV['TRAVIS_BRANCH']}-valid-batch-name",
  ...
  it 'should persist jars and output all successfully validated records with the same key, headers, and body after deleting the flink jobmanager pod' do
    ...
    raise "Jar with name 'flink-validation-fhir-nightly-test-jar.jar' not found" unless jar_found
